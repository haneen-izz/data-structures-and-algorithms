...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let result = input.map((value) => {
    array = [];
    value.forEach((secValue) => {
      if (secValue % 5 === 0 && typeof secValue === 'number') {
        array.push(Math.pow(2, secValue));
      }
    });
    return array;
  });
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let dataRes = data.reduce((accumulator, value) => {
    if (value.gender === 'female' || value.gender === 'male') {
      if (accumulator === '') {
        accumulator = value.name;
      } else {
        accumulator = accumulator + ' and ' + value.name;
      }
    }
    return accumulator;
  }, '');
...
let findShortest = (data) => {
  // Solution code here...
  let Array = data.sort((firstEl, secEl) => {
    if (firstEl.height > secEl.height) {
      return -1;
    } else if (firstEl.height < secEl.height) {
      return 1;
    } else {
      return 0;
    }
